...
- Есть комментарий к API (XML/Javadoc/аналог по стандарту проекта) с описанием будущей логики?
...
- Есть комментарии к API для новых классов и методов (XML/Javadoc/аналог по стандарту проекта)?
- Имена переменных и функций понятны?
- Сложная логика разбита на методы?
- Код следует стандартам проекта (C#/.NET conventions, Java conventions и т.д.)?

**Пример проблемы:**
```
❌ Метод processPayment() не имеет XML/Javadoc комментария
...
**Пример проблемы:**
```
❌ Добавлен файл DiscountService.cs (или DiscountService.java), но он не упомянут в src/services/.AGENTS.md
...
- Отсутствуют XML/Javadoc комментарии для новых методов
- Дублирование кода
...
1. **[Краткое описание проблемы]**
   - **Файл:** `path/to/file.cs` (или `path/to/file.java`)
...
```
🔴 Критичное: Не реализован метод refundPayment()
   - Файл: src/services/PaymentService.cs
   - Проблема: В описании задачи (task_2_3.md, раздел "Описание изменений") 
     указано добавить метод refundPayment(string paymentId): bool в класс PaymentService,
     но этот метод отсутствует в коде
   - Требуемое исправление: Добавить метод согласно описанию задачи

🟡 Важное: Отсутствует XML/Javadoc комментарий для метода applyDiscount()
   - Файл: src/services/OrderService.cs, строка 45
   - Проблема: Метод applyDiscount() не имеет комментария с описанием параметров и возвращаемого значения
   - Рекомендация: Добавить XML/Javadoc комментарий по образцу других методов класса

🟢 Рекомендация: Можно упростить проверку userLevel
   - Файл: src/services/DiscountService.cs, строки 23-30
   - Рекомендация: Вместо цепочки if/else можно использовать словарь/Map ставок по уровню пользователя
```
