...
| Роль | Кто | Назначение |
| --- | --- | --- |
| `restrict_ui` | VTBL.Restrict.UI | Загрузки, чтение справочников, resolve кейсов обработки ошибок |
...
| ErrorProcessingCase | SELECT; UPDATE при resolve (Status, ResolvedAt, ResolvedBy) | INSERT, SELECT, UPDATE (Status Expired/Cancelled) | ALL |
| ErrorProcessingItem | SELECT; UPDATE (UserValue) | INSERT, SELECT | ALL |

## Правила приложения (обязательные)

1. UI **не** создаёт `ErrorProcessingCase` / `ErrorProcessingItem`.
...
- Для ужесточения: stored procedures `usp_ErrorProcessing_GetById`, `usp_ErrorProcessing_Resolve` под UI; парсер — `usp_ErrorProcessing_Create` (фаза 2, не блокер MVP).
