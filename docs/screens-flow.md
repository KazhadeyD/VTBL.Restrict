...
**Дата:** 14.07.2026  
**Визуал особого случая** — placeholder; детальный UI обсуждается отдельно.
...
| Особый случай | `/special-cases/{caseId}?token=...` | UC-03 |
...
## 3. Special Case (placeholder layout)
...
3. Таблица/форма по `SpecialCaseItem` (порядок `SortOrder`):
   - FieldCode / подпись
   - RawValue (read-only)
...
Отдельное состояние той же страницы или `/special-cases/invalid`:
...
На MVP достаточно Upload в меню; Special Case открывается только из письма (можно не светить в navbar).
